 ...
 */
export async function zipTheDirectory(sourceZipDirectory) {
 ...
 */
export async function zipAndMoveDirectory(sourceZipDirectory, destinationZipDirectory) {
	try {
		...
		try {
			...
			if (directoryNotEmpty) {
				...
				if (runZipAndMove) {
					...
					await zipAndMoveDirectory(sourceZipDirectory, destinationZipDirectory);
				}
				...
				logger.info(`Scheduled cycle skipped. Directory ${sourceZipDirectory} is empty.`);
			}
		} catch (error) {
